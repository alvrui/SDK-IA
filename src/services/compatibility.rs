...
impl CompatibilityMatrix {
    ...
    fn natural_fit(&self, a: &HollywoodElement, b: &HollywoodElement) -> f32 {
        if matches!(a.category, ElementCategory::Finale) || matches!(b.category, ElementCategory::Finale) {
            return self.setting_fit(a, b);
        }
    ...
    fn reinterpretation_fit(&self, a: &HollywoodElement, b: &HollywoodElement) -> f32 {
        if (a.id == "KNIGHT" && b.setting_affinity.iter().any(|s| s.contains("MODERN"))) ||
           (b.id == "KNIGHT" && a.setting_affinity.iter().any(|s| s.contains("MODERN"))) { return 0.85; }
        if (a.id == "ROBOT" && b.setting_affinity.iter().any(|s| s.contains("MEDIEVAL"))) ||
           (b.id == "ROBOT" && a.setting_affinity.iter().any(|s| s.contains("MEDIEVAL"))) { return 0.30; }
        if (a.id == "COWBOY" && b.setting_affinity.iter().any(|s| s.contains("SPACE"))) ||
           (b.id == "COWBOY" && a.setting_affinity.iter().any(|s| s.contains("SPACE"))) { return 0.60; }
        0.5
    }
    
    fn novelty_bonus(&self, a: &HollywoodElement, b: &HollywoodElement) -> f32 {
        if (a.id == "ACCIDENTAL_HERO" && b.id == "ANCIENT_EVIL") || (a.id == "ANCIENT_EVIL" && b.id == "ACCIDENTAL_HERO") { return 0.10; }
        if (a.id == "ROBOT" && b.setting_affinity.iter().any(|s| s.contains("DYSTOPIAN"))) ||
           (b.id == "ROBOT" && a.setting_affinity.iter().any(|s| s.contains("DYSTOPIAN"))) { return 0.15; }
        if (a.id == "CLUMSY_OAF" && b.tone_profile.serious > 0.7) || (b.id == "CLUMSY_OAF" && a.tone_profile.serious > 0.7) { return 0.05; }
        0.0
    }
    ...
    fn sensitivity_risk(&self, a: &HollywoodElement, b: &HollywoodElement) -> f32 {
        if (a.tone_profile.comic > 0.5 && b.content_flags.iter().any(|f| f.contains("extreme_violence"))) ||
           (b.tone_profile.comic > 0.5 && a.content_flags.iter().any(|f| f.contains("extreme_violence"))) { return 1.0; }
        if (a.tone_profile.serious > 0.7 && b.genre_affinity.iter().any(|g| g.contains("comedy"))) ||
           (b.tone_profile.serious > 0.7 && a.genre_affinity.iter().any(|g| g.contains("comedy"))) { return 0.8; }
        if a.moral_profile.idealist > 0.7 && b.moral_profile.corrupt > 0.7 { return 0.7; }
        if (a.tone_profile.grim > 0.7 && b.tone_profile.comic > 0.5) || (b.tone_profile.grim > 0.7 && a.tone_profile.comic > 0.5) { return 0.6; }
        if (a.tone_profile.melodramatic > 0.7 && b.tone_profile.pulpy > 0.7) || (b.tone_profile.melodramatic > 0.7 && a.tone_profile.pulpy > 0.7) { return 0.3; }
        0.0
    }
    ...
    fn generate_explanation(&self, a: &HollywoodElement, b: &HollywoodElement, axis_scores: &HashMap<String, f32>, penalties: f32, bonuses: f32) -> Vec<String> {
        let mut explanations = Vec::new();
        for (axis, score) in axis_scores {
            if *score >= 0.8 {
                match axis.as_str() {
                    "role_complementarity" => explanations.push(format!("ROLE_COMPLEMENTARITY: {} and {} have complementary narrative roles", a.id, b.id)),
                    "moral_tension" => explanations.push(format!("MORAL_TENSION: Strong ethical contrast between {} and {}", a.id, b.id)),
                    "tone_alignment" => explanations.push(format!("TONE_ALIGNMENT: {} and {} share similar tonal profiles", a.id, b.id)),
                    "embodiment_strength" => {
                        let event_id = if matches!(b.category, ElementCategory::Event) { b.id.as_str() } else { a.id.as_str() };
                        let theme_id = if matches!(a.category, ElementCategory::Theme) { a.id.as_str() } else { b.id.as_str() };
                        explanations.push(format!("EMBODIMENT: {} strongly embodies the theme of {}", event_id, theme_id));
                    }
                    "arc_payoff" => {
                        let finale_id = if matches!(b.category, ElementCategory::Finale) { b.id.as_str() } else { a.id.as_str() };
                        let theme_id = if matches!(a.category, ElementCategory::Theme) { a.id.as_str() } else { b.id.as_str() };
        ...
        if bonuses > 0.0 {
            if (a.id == "ACCIDENTAL_HERO" && b.id == "ANCIENT_EVIL") || (a.id == "ANCIENT_EVIL" && b.id == "ACCIDENTAL_HERO") {
