...
class HollywoodAnimalCatalog:
    ...
    def _genre_support(self, a: HollywoodElement, b: HollywoodElement) -> float:
        a_genres, b_genres = set(a.genre_affinity), set(b.genre_affinity)
        intersection, union = len(a_genres & b_genres), max(len(a_genres | b_genres), 1)
        return min(intersection / union, 1.0)

    def _setting_fit(self, a: HollywoodElement, b: HollywoodElement) -> float:
        a_settings, b_settings = set(a.setting_affinity), set(b.setting_affinity)
        intersection, union = len(a_settings & b_settings), max(len(a_settings | b_settings), 1)
        return min(intersection / union, 1.0)
    ...
    def _arc_coherence(self, a: HollywoodElement, b: HollywoodElement) -> float:
        if (a.category == 'protagonist' and b.category == 'theme') or (a.category == 'theme' and b.category == 'protagonist'):
            ...
            if 'love' in theme.id.lower() and 'love' in p_drives: return 0.95
            if 'justice' in theme.id.lower() and 'justice' in p_drives: return 0.95
            if 'freedom' in theme.id.lower() and 'freedom' in p_drives: return 0.95
            if 'war' in theme.id.lower() and 'survival' in p_drives: return 0.90
            if 'treasure' in theme.id.lower() and 'greed' in p_drives: return 0.85
            return 0.6
    ...
    def _core_drives_overlap(self, a: HollywoodElement, b: HollywoodElement) -> float:
        a_drives, b_drives = set(a.core_drives), set(b.core_drives)
        intersection, union = len(a_drives & b_drives), max(len(a_drives | b_drives), 1)
        return min(intersection / union, 1.0)
